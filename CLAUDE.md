...
- **Migration workflow (prevents history drift — non-negotiable)**: The remote migration history repeatedly desynced because migrations were applied out-of-band. To keep `supabase db push` working: (1) **CREATE** migrations only with `supabase migration new <name>` — it stamps a unique 14-digit `YYYYMMDDHHMMSS` version. NEVER hand-name an 8-digit `YYYYMMDD` version or reuse a number (both break the CLI's local↔remote matching). (2) **APPLY** migrations only with `npx supabase db push` (records history). Do NOT apply schema via the Supabase dashboard SQL editor or `supabase db query` for anything you want tracked. (3) If you ever hotfix out-of-band, immediately record it: `supabase migration repair --status applied <version>`. (4) Before a DB release, `npx supabase migration list --linked` should show no blank cells. Full root-cause writeup + recovery runbook: `tasks/migration-history-reconcile.md`.

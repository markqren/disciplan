...
#### v2.10.12
<sub>Newsletter structural fixes: budget_pace trip tags + no home projection; streak parent rollup; flashback reimbursements; SQL audit log</sub>
...
##### Fixes
- **Newsletter cost cut ~70-90% on tool-heavy sends via prompt caching (FEA-116)** — Cost had doubled (avg $0.020→$0.045/send, spiking to **$0.165** for a 2026-07-09 `budget_pace` that ran ~36k input tokens) because `generateInsightText` ran a multi-turn `run_finance_query` loop that re-sent the entire growing context **uncached** every turn, re-billing the full prompt + facts + tool definition at 1x per round-trip. The static prefix (tool def + the day's prompt/facts/guidance) is now marked Anthropic `cache_control: ephemeral`, so each tool-loop turn re-reads it at 0.1x instead of 1x (the 5-min cache TTL easily spans one generation's seconds-long loop). Cost is now computed exactly from the `cache_creation`/`cache_read`/`input`/`output` token split (writes 1.25x, reads 0.1x). Mark flagged this directly: "we're hitting 11 cents a newsletter" / "it cost 16 cents to produce?". (~4,000 tokens)
- **Follow-up questions no longer "cap out" (FEA-116)** — Follow-ups kept deferring to "tomorrow" ("I hit my query limit trying to pull… so I wasn't able to"), because the main insight and the follow-up shared one `MAX_TOOL_CALLS=4` budget and the main insight's drill-downs exhausted it first. With caching making extra turns cheap, the budget is raised to 8 calls / 10 turns and the writer is instructed to run the follow-up's queries FIRST (they're already flagged highest-priority), so a follow-up is answered with real numbers this send instead of being punted. (~2,000 tokens)
- **Rakuten forwarding notes now survive HTML-only forwards** — Forwarded Rakuten cashback emails (whose merchant appears only as a logo image, so the merchant must be typed into the forward) were landing as `Rakuten - Unknown Store`: the inbound-email function's `extractForwardingNote` read only the `text/plain` body, which Apple Mail ships degraded/empty for these HTML notices, so the note was dropped and the merchant-resolving AI path was skipped. It now falls back to an HTML→text conversion (`htmlToText`, with divider-safe line-break normalization) when the plaintext body has no forwarding divider, so a `Merchant: Priceline` note is read and the import resolves to `Rakuten - Priceline`. (~3,000 tokens)
- **Rakuten cashback auto-proposes its parent purchase with a confidence bar** — Auto-linking never fired for note-driven imports because `findRakutenParentMatch` searched on `parsed_data.store_name`, which held the raw forwarding-note blob (`Merchant: Priceline (Comfort Inn Hotel) Category: Accommodation`) rather than a clean merchant, so the ledger `ILIKE` never matched. `cleanRakutenStore` now strips `Merchant:`/`Category:` labels, parentheticals, and extra lines to recover `Priceline`, and the matcher scores each in-window candidate (name + order-total amount + date proximity + lone-match bonus) and only proposes above a bar (`RAKUTEN_MATCH_BAR`, medium/high confidence). The review row and edit modal surface the suggested parent and its confidence for one-click approve / unlink. (~3,500 tokens)

##### Features
- **Newsletter cost observability (FEA-116)** — `insight_log` gained `tool_calls`, `cache_read_tokens`, and `cache_write_tokens` (migration `20260709222625_insight_log_cost_observability.sql`), populated on every send. The AI portal's newsletter tab adds an "avg cost/send (last 10)" KPI (green ≤$0.03 / yellow ≤$0.06 / red above) and per-row badges showing the query count (`Nq`) and a `cached` marker, so cost regressions are visible at a glance instead of inferred. (~2,500 tokens)
...
|----|------|------|-----------|
| FEA-125 | **Elect a ledger credit as cashback** — Negative-amount rows carry a **🏆 Cashback** checkbox beside Subscription in the edit modal. Ticking it writes a `cashback_redemptions` row inferred from the transaction (date, description, payment account, `\|amount\|` as Dollar Value) so the credit shows on the Cashback tab with no second income transaction; unticking removes it. Both directions are covered by the edit-undo toast, and tagged rows show a trophy in the ledger list. The Cashback *button* keeps its original FEA-14 meaning — mint a new cashback credit linked to a purchase — and is hidden on credits. Points/rate stay editable from the Cashback tab. | Feature → Done | Aug 31 |

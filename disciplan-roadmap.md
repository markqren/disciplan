...
| FEA-24 | **CSV Transaction Import** | Feature | High | Upload a bank/credit card CSV statement and auto-translate each line item into the Disciplan ledger format. Flow: (1) User uploads CSV via file picker, (2) parser detects columns (date, description, amount, etc.) and maps them to schema fields, (3) AI/heuristic matching normalizes descriptions to user's existing style (e.g. "UBER EATS" → "Uber Eats delivery", categories auto-assigned based on historical patterns), (4) each proposed transaction is shown one-by-one in a confirmation modal where user can edit any field before approving, (5) approved items are POSTed to Supabase. Needs: CSV column auto-detection for major banks (Chase, AMEX, Schwab, etc.), duplicate detection against existing transactions (by date + amount + description similarity), service period defaults from `ACCRUAL_D`, payment type auto-set from the source CSV/account. Deferred items should be skippable and resumable. |
